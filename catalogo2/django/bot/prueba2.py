...
for x in divisa:
    ...
    #hora_inicial = f"{fecha_actual} 6:00:00.00"
    hora_inicial = f"2024-01-04 6:00:00.00"
    datetime_object = datetime.strptime(hora_inicial, "%Y-%m-%d %H:%M:%S.%f")
    fecha_proxima = datetime_object.date() + timedelta(days=1)
    ...
    for i in range(rango):
        ...
        fecha_solo = fecha_solo - timedelta(days=1)


        hora_inicial2 = f"{fecha_solo} 21:00:00.00"
    ...
    for i in lista:
            ...
            if hora_minuto_segundo not in horas_procesadas:
                    ...
                    if porcentaje_alcistas >= porciento_total and doji <= dojis_permitidos and total_velas >= velas_permitidas:
                            if datetime.strptime(hora_minuto_segundo, '%H:%M:%S') < datetime.strptime('21:00:00', "%H:%M:%S"):
                                    fecha_final = f"{fecha_proxima} {hora_minuto_segundo}"
                                   
                            else:   
                                    #fecha_final = f"{datetime.now().date()} {hora_minuto_segundo}"
                                    fecha_final = f"2024-01-04 {hora_minuto_segundo}"
